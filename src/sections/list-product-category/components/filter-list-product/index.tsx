...
export default function FilterListProductCategory(props: IProps) {
  const {
    listAttribute,
    searchParams,
    slug,
  ...
  const [dataInit, setDataInit] = useState<any>(dataListProductInit ?? []);
  ...
  const [dataPagination, setDataPagination] = useState<IDataPagination>({
    total: dataInit.countItem,
    currentPage: 1,
    perpage: 12,
  });

  const bodyGetListProduct: any = {
    url: `wp-json/product/v1/products/filter?category=${slug}&per_page=12&page=${dataPagination.currentPage}&${paramRouterGetApi}`,
    method: 'get',
  };

  const getlistProduct = useSWR(bodyGetListProduct.url, () =>
    paramRouterGetApi || dataPagination.currentPage > 1
  ...
  // }
  // getlistProduct.mutate();
  // }, [paramsFilter, getlistProduct.data, dataPagination]);
    ...
    <div className="filter-list-product-container">
                ...
                item.listAttribute.map((itemAttribute: any, index: number) => (
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(value: boolean) =>
                        onChange(itemAttribute.taxonomy, itemAttribute.slug)
                      }
                      id="1"
          ...
          {getlistProduct.isLoading ? (
            ...
            <div>
              <div className="grid grid-cols-4 gap-4">
                {dataInit &&
                  dataInit?.item?.length > 0 &&
                  dataInit?.item?.map((item: any, index: number) => (
      ...
      <div className="hidden max-md:block">
        <div className="grid grid-cols-2">
          {map(dataInit?.item, (item: any, index: number) => (
