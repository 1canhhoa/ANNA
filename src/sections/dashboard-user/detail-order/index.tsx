...
function DetailOrder(props: IProps) {
  ...
  const [dataDetailOrder, setDataDetailOrder] = useState<any>();
  ...
  const handleConvertAddress = () => {
    const company = `${dataDetailOrder?.shipping?.company}${
      dataDetailOrder?.shipping?.company ? ',' : ''
    }`;
    const address1 = `${dataDetailOrder?.shipping?.address_1}${
      dataDetailOrder?.shipping?.address_1 ? ',' : ''
    }`;
    const address2 = `${dataDetailOrder?.shipping?.address_2}${
      dataDetailOrder?.shipping?.address_2 ? ',' : ''
    }`;
    const district = `${dataDetailOrder?.shipping?.state}${
      dataDetailOrder?.shipping?.state ? ',' : ''
    }`;
    const city = `${dataDetailOrder?.shipping?.city}${
      dataDetailOrder?.shipping?.city ? '.' : ''
    }`;
    const country = `${dataDetailOrder?.shipping?.country}${
      dataDetailOrder?.shipping?.country ? '.' : ''
    }`;
    ...
    <div className="detail-order-dashboard">
        ...
        <div className="info-order-dashboard w-[27.4375rem] h-[5.8125rem] flex justify-between items-center px-[1rem] rounded-[1rem] max-md:w-full max-md:h-[14rem] max-md:rounded-[10rem] max-md:px-[4.93333rem] max-md:leading-[3rem]">
          <div>
            <div className="flex items-center max-md:hidden">
              ...
              <h3 className="ml-[0.2rem] leading-[1.5rem] text-[0.875rem] not-italic font-medium max-md:text-[2.61rem] max-md:leading-[3rem]">
                {/* 03.07.2022, 10:01 AM */}
                {dataDetailOrder?.date_created}
              </h3>
            </div>
            <div className="items-center hidden max-md:flex ">
              <ICPhone stroke="black" height="3rem" width="3rem" />
              <h3 className="leading-[1.5rem] text-[0.875rem] not-italic font-medium max-md:text-[2.61rem] max-md:leading-[3rem] ml-[2rem]">
                {/* 03.07.2022, 10:01 AM */}
                {dataDetailOrder?.order?.date_created?.date}
              </h3>
            </div>
            <div className="flex max-md:mt-[2.13rem]">
              <h3 className="ml-[0.2rem] leading-[1.5rem] text-[0.875rem] font-medium not-italic font-Nexa-Normal max-md:text-[2.61rem] max-md:leading-[3rem]">
                Đơn hàng ID:{' '}
              </h3>
              <h3 className="ml-[0.2rem] leading-[1.5rem] text-[0.875rem] font-medium not-italic font-Nexa-Normal max-md:text-[2.61rem] max-md:leading-[3rem] ">
                {dataDetailOrder?.id}
          ...
          <div className="w-[6.25rem] h-[1.875rem] bg-[#D9F3E2] flex justify-center font-semibold items-center text-[0.75rem] not-italic rounded-[0.25rem] text-[#00AD3B] max-md:text-[2.61rem] max-md:h-[7rem] max-md:w-[15rem] max-md:rounded-[4rem]">
        ...
        <div className="flex justify-between mt-[1.5rem]">
          <div className="w-full pr-[1rem]">
            ...
            <div className="body-list-product">
              {dataDetailOrder ? (
                map(dataDetailOrder?.product, (item: any, index: number) => (
                  <div key={index} className="row-body-list-product">
                    ...
                    >
                      <div className="cursor-pointer max-md:text-[3.33rem] underline max-md:leading-[4.5rem] text-blueAnna">
                        Mua lại
                      </div>
                    </Link>
                  </div>
      ...
      <div className="flex justify-between max-md:flex-col">
        <div className="w-[60%] px-[1.5rem] py-[1rem] bg-[#F6FDFD] rounded-[0.5rem] mt-[1.5rem] max-md:w-full max-md:py-[3rem] max-md:px-[3rem] max-md:rounded-[4rem]">
          <h3 className="text-[1rem] not-italic font-bold mb-[0.7rem] max-md:text-[3.78rem] max-md:font-Nexa-Bold max-md:mb-[2rem]">
          ...
          <div className="mb-[0.75rem] flex justify-between">
            ...
            <span className="text-[0.875rem] leading-[1.3125rem] font-semibold not-italic max-md:text-[2.88rem] max-md:leading-[5.6rem]">
              {dataDetailOrder?.payment_method_title}
            </span>
          </div>
          <hr className="bg-[#E5EAEA]" />
          <div className="mb-[0.25rem] flex justify-between mt-[1rem]">
            ...
            <span className="text-[0.875rem] leading-[1.3125rem] font-extrabold not-italic max-md:text-[2.88rem] max-md:leading-[5.6rem]">
